...
- **Domain** (`app/models`):
  ...
  - **Newspaper.** The newspaper component is a whole front page in one tile, meant to fill the grid (`renders/items/_newspaper`, `newspaper/_story`, `.paper-*` in `render.css`). It takes RSS feeds and a weather source together. The masthead is its `name` in UnifrakturMaguntia with a weather ear. The lead is the newest story with a photo; the left column opens with the next photo story, the right with a big headline from another source, and briefs follow with sources taking turns (`NewsFeed.by_turns`), some with thumbnails. Headlines are never clipped: an inline script in the partial runs once fonts load, stepping each box's first headline down its `NewspaperLayout` sizes (`.hl-<px>`) until it fits, then keeping later stories only while they fit, and shrinking the name to fit between the ears. Its fonts (Newsreader, Oswald, UnifrakturMaguntia, OFL texts in `app/assets/fonts/licenses`) are woff2 files inlined like the rest. Parts: weather, dateline, photos, bylines, summaries. `_item` gives every card a `card--<kind>` class, which the newspaper uses to drop its border. Aggregator headlines lose the outlet suffix their `<source>` names.
