...
- **Domain** (`app/models`):
  ...
  - **Newspaper.** The newspaper component is a whole front page in one tile, meant to fill the grid (`renders/items/_newspaper`, `newspaper/_story`, `.paper-*` in `render.css`). It takes RSS feeds, a weather source and calendars together. The masthead is its `name` in blackletter with a weather ear. The lead is the newest story with a photo; the left column opens with the next photo story, the right with a big headline from another source, and briefs follow with sources taking turns (`NewsFeed.by_turns`), some with thumbnails. An "Upcoming events" box opens the right column with the calendars' events under way or starting within its `event_hours` (36 by default), labeled by `NewsHelper#newspaper_event_when`. Headlines are never clipped: an inline script in the partial runs once fonts load, stepping each box's first headline down its `NewspaperLayout` sizes (`.hl-<size>`, each a face and size) until it fits, then dropping the events box's latest events, then keeping later stories only while they fit. Stories that fit nowhere then fill the gaps at the foot of either column, cut down to their smallest headline (with a byline if there's room). It also shrinks the name to fit between the ears. Its faces are pixel fonts, each set only at a whole multiple of its grid (noted on its `@font-face`): Jacquard 12/24 for the name, Jersey 15/20/25 for headlines, Pixel Operator and Bitrimus for small headlines, PixAntiqua for text, Pixeloid Sans for labels. They're OFL or CC0 (texts in `app/assets/fonts/licenses`), stored as unmodified woff2 (no subsetting: some reserve their names under the OFL) and inlined like the rest. Parts: weather, dateline, photos, events, bylines, summaries. `_item` gives every card a `card--<kind>` class, which the newspaper uses to drop its border. Aggregator headlines lose the outlet suffix their `<source>` names.
